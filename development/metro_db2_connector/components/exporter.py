...
* check in DB2 if the record has been updated more recently than the
  last sync date and if yes, delay an import
* call the ``bind`` method of the binder to update the last sync date
"""


class DB2BaseExporter(AbstractComponent):
    """ Base exporter for DB2 """

    _name = 'db2.base.exporter'
    _inherit = ['base.exporter', 'base.db2.connector']
    _usage = 'record.exporter'

    def __init__(self, working_context):
        super(DB2BaseExporter, self).__init__(working_context)
        self.binding = None
    ...
    def _delay_import(self):
        ...
        self.binding.with_delay(eta=2).import_record(self.backend_record,
                                                self.external_id,
                                                force=True)

    def _should_import(self):
        """ Before the export, compare the update date
        in DB2 and the last sync date in Odoo,
        if the former is more recent, schedule an import
        to not miss changes done in DB2.
        """
        assert self.binding
        ...
        if not sync:
            return True
        record = self.backend_adapter.read(self.external_id, attributes=['last_updated_attribute'])
        if not record['last_updated_attribute']:
            # in rare case it can be empty, in doubt, import it
            return True
        sync_date = odoo.fields.Datetime.from_string(sync)
        db2_date = datetime.strptime(record['last_updated_attribute'], 'Y-m-d')
        return sync_date < db2_date
    ...
    def _after_export(self):
        """ Can do several actions after exporting a record in DB2 """
        pass


class DB2Exporter(AbstractComponent):
    """ A common flow for the exports to DB2 """

    _name = 'db2.exporter'
    _inherit = 'db2.base.exporter'

    def __init__(self, working_context):
        super(DB2Exporter, self).__init__(working_context)
        self.binding = None
    ...
    def _retry_unique_violation(self):
        ...
        record created by :meth:`_export_dependency`)
        """
        try:
            yield
    ...
    def _export_dependency(self, relation, binding_model,
                           ...
                           binding_field='db2_bind_ids',
                           binding_extra_vals=None):
        """
        Export a dependency. The exporter class is a subclass of
        ``DB2Exporter``. If a more precise class need to be defined,
        ...
        if wrap and hasattr(relation, binding_field):
            ...
            # db2.product.product and we are exporting it, but we need to
            ...
            # If db2_bind_ids does not exist we are typically in a
    ...
    def _run(self, fields=None):
        ...
        return _('Record exported with ID %s in DB2.') % self.external_id
