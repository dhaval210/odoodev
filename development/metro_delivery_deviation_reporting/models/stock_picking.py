...
class StockPicking(models.Model):
    ...
    def _deviations_cron_action(self):
        ...
        today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(hours=float(threshold))
        ...
        for order in orders:
            ...
            if wh.delivery_steps == "pick_pack_ship":
                ...
                for pick in order.picking_ids:
                    ...
                    if pick.picking_type_id.id == wh.pack_type_id.id:
        ...
        for pick in pickings:
            ...
            if delay:
                if pick.state == "done" and pick.date_done:
                    ...
                    date = "Not packed yet"
                deviation_details["time"].append({"name": pick.name, "date": date, "latest_pack_date": latest_pack_date})
                deviation_details["operations"] += pick.name + ", "
                time_delay = True

        if time_delay:
            # Check done pickings
            order_lines, missing_product_ids = self.check_missing_products(order_lines, done_pickings)
    ...
    def check_delivery_date(self, pick, threshold):
        latest_pack_date = pick.customer_delivery_date - timedelta(float(threshold) / 24)
    ...
    def check_quantities(self, order_lines, done_pickings):
        ...
        for key in order_lines.keys():
            ...
            if not order_lines[key]["qty"] == done_pickings[key]["qty_done"]\
                or not order_lines[key]["uom_id"] == done_pickings[key]["uom_id"]:
    ...
    def send_mail(self, order, time_delay, deviation_details):
        ...
        <br />
        The order """+ str(order.name) +""" has some delivery deviations.<br />"""
        product_details = []
        ...
        if time_delay:
            ...
            """ ("""+time_zone.zone+""")"""+"""<br/>Latest moment in time of packing: """+ latest_pack_date.strftime("%Y-%m-%d %H:%M:%S") +\
            ...
            for item in deviation_details["time"]:
                if type(item["date"]) != type("string"):
                    ...
                    msg += "<li>"+item["name"]+": "+ date.strftime("%Y-%m-%d %H:%M:%S") + " ("+time_zone.zone+")" + "</li>"
                else:
                    msg += "<li>"+item["name"]+": "+ item["date"] + "</li>"
        ...
        if len(deviation_details["missing"]) >= 1:
            ...
            products = self.env["product.product"].browse(list(deviation_details["missing"].keys()))
            for item in products:
                ...
                detail["qty_done"] = 0
                detail["qty_ordered"] = deviation_details["missing"][item.id]["qty_ordered"]
                detail["uom_id"] = deviation_details["missing"][item.id]["uom_id"]
        ...
        if len(deviation_details["changed_qty"]) >= 1:
            ...
            products = self.env["product.product"].browse(list(deviation_details["changed_qty"].keys()))
            for item in products:
                default_code = item.product_tmpl_id.default_code or "EMPTY"
                
                msg += "<li> [" + default_code + "] "+ item.product_tmpl_id.name + " Ordered: "+\
                    str(deviation_details["changed_qty"][item.id]["qty_ordered"]) + " -> "+\
                    str(deviation_details["changed_qty"][item.id]["qty_done"]) +"</li>"
                detail = {}
                detail["product_id"] = item.id
                detail["qty_done"] = deviation_details["changed_qty"][item.id]["qty_done"]
                detail["qty_ordered"] = deviation_details["changed_qty"][item.id]["qty_ordered"]
                detail["uom_id"] = deviation_details["changed_qty"][item.id]["uom_id"]
                product_details.append(detail)
        ...
        if time_delay or len(deviation_details["changed_qty"]) >= 1\
            ...
            # Generate mail
            subject = "Delivery Deviation for " + deviation_details["operations"]
