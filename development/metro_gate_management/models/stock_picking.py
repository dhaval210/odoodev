...
class StockPicking(models.Model):
    ...
    gate_id = fields.Many2one(
        ...
        default=lambda self: self.env['stock.gate'].search([('name', '=', 'Undefined')])
    ...
    def init(self):
        record = self.env['stock.gate'].search([('name','=','Undefined')])
        if not record:
            record = self.env['stock.gate'].create({
                'name': 'Undefined',
                'active': True,
                'state': False,
            })
        gate = record
        self.env.cr.execute("""
        UPDATE stock_picking SET gate_id=%s WHERE gate_id IS NULL
        """% gate.id)
    ...
    def _onchange_gate_id(self):
        ...
        if self.gate_id.id is not False:
            if len(pickings):
                if not self.gate_id.name == 'Undefined':
