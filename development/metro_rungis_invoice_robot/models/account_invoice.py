...
class AccountInvoice(models.Model):
    ...
    def attach_pdf_file_to_send(self, partner):
        """
            creating attachement report to send to partner
        """
        # TODO: Optimization potential!!
        # render_qweb_pdf calls embed_ubl_xml_in_pdf through _post_pdf and some other functions
        # if invoice_format is pdf_ubl it'll be called again to insert the UBL content into the PDF
        # NOTE: Maybe the one attachment file is creating in a sub function from render_qweb_pdf
        ...
        pdf = base64.b64encode(file_id)
    
        name = self.number if self.number and self.number.endswith("pdf") else self.number + ".pdf"
        # NOTE: The 2nd attachment would be created here, if suspicion is true now no 2nd attachment should be created
        # testing locally: True, no 2nd attachment was created, but thats the attachment we actually want to send
        """
    ...
    def attach_ubl_xml_file_to_send(self, partner):
        """
            creating attachement EDI, PDF w/ embedded UBL to send to partner
        """
    ...
    def send_invoice_mail(self, partner):
        ...
        template = None
        # NOTE: Somewhere in this function the attachment from Send & Print is created, w/o calling this method it won't exist
        data_id = self.generate_attachment_file(partner)
        # Temporary fix for finding PDF which is created as a sideeffect inside generate_attachment_file
        if not data_id:
            data_id = self.env["ir.attachment"].search([
        ...
        # NOTE: This part throws an error if auto deletion is turned on for sent mails
        # mail = self.env["mail.mail"].browse([mail_id])
        # mail.recipient_ids = [(6, 0, [partner.id])]
        return True
    
    def generate_attachment_file(self, partner):
        data_id = None
        # PDF w/ or w/o embedded XML
        # NOTE: This is currently NOT called in send_invoice_mail
    ...
    def _check_invoice_errors(self):
        ...
        for line in self.invoice_line_ids:
            if line.price_unit <= 0.10 or (line.price_unit >= 999.0 and line.price_unit < 1000.0):
                message += "- The price is not well set: " + str(line.price_unit) + "<br />"
            elif not line.invoice_line_tax_ids:
                message += "- The tax is not set<br/>"
            elif not line.product_id.hs_code_id:
                message += "- The HS_Code is not set for product {}<br/>".format(line.product_id.name)
            elif line.invoice_line_tax_ids:
                for tax in line.invoice_line_tax_ids:
                    if tax.is_fake:
                        message += "- The tax {} is marked as a fake tax.<br/>".format(tax.name)
                        break
        return message
