    ...
    def generate_check_invoice(self):
        ...
        for company in company_ids:
            ...
            for partner in partner_ids:
                ...
                if not inv_partner:
                    inv_partner = partner
                # NOTE: returned as a list in a list for some reason
                err = inv_partner._check_valid_process_controls()[0]
                if len(err) > 0:
                    logger.warn("There was an error with the contact {} ({})".format(inv_partner.name, inv_partner.id))
                    msg = "<p>There were some errors while trying to generate invoices for this partner:</p><ul>"
                    for e in err:
                        msg += "<li>" + e + "</li>"
                    msg += "</ul>"
                    inv_partner.message_post(body=msg)
                    continue
