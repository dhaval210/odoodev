...
class WizardValueReport(models.AbstractModel):
    ...
    _name = "report.metro_barcode_print_community.print_package_contents"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        package_ids = [index['result_package_id'] for index in data['package_content_moves']]
        for package_moves in data['package_content_moves']:
            package_moves['print_copy'] = [i for i in range(0, int(
                package_moves['print_copy']))]
        stock_package_content_id = self.env['stock.quant.package'].search([(
            'id', '=', package_ids)])
        return {
            'doc_model': 'content.barcode.report',
            'docs': stock_package_content_id,
            'data': data,
            'same_page': same_page,
