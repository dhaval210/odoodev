...
class CacheOrderGrid(models.Model):
    ...
    def sum_daily_diff(self, data):
        ...
        if (
            ...
            data['outgoing_planned_qty'] < data['removal_qty']
        ):
            sum = sum - (data['removal_qty'] - data['outgoing_planned_qty'])
        return sum
    ...
    def get_removal_qty_from_lot(self, single_date, data, warehouse):
        ...
        if len(lots) > 0:
            quants = self.env["stock.quant"].search([
                ...
                ('lot_id', 'in', lots.mapped('id'))
            ])
            if len(quants) > 0:
                removal_qty = sum(quants.mapped('quantity'))
        data.update({
    ...
    def get_transit_qty(self, single_date, operation_ids, product):
        ...
        something = sum(moves.mapped('product_uom_qty'))
        return something
    ...
    def job_populate_supplier(self, supplier, whid, data, force_update=False, start_date=False, end_date=False):
        ...
        for single_date in self.daterange(start_date, end_date):
            ...
            if single_date in existing_dates:
                eduid = existing_data.filtered(lambda x: x.date == single_date)
                eduid.write(data)
            else:
                self.create(data)
