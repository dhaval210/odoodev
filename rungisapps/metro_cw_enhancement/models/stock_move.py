...
from odoo.addons.tis_catch_weight.models.stock_move import StockMove as StockMoveModel
...
class StockMove(models.Model):
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        if (
            ...
            not self.product_id._is_cw_product()
        ):
            return super(StockMoveModel, self)._prepare_move_line_vals(quantity=quantity, reserved_quant=reserved_quant)
        else:
            res = super(StockMoveModel, self)._prepare_move_line_vals(quantity=quantity, reserved_quant=reserved_quant)
                    ...
                    if self.product_id.tracking == 'serial':
                        if cw_params and self.id in cw_params.keys():
                            ...
                            if isinstance(quantity_list, list) and quantity_list:
                                ...
                                if cw_params and self.id in cw_params.keys():
                                    serial_qty = cw_params.get(self.id)
                                    res.update({'product_cw_uom_qty': serial_qty})
                    else:
                        if cw_params and self.id in cw_params.keys():
                            cw_qty = cw_params.get(self.id)
                            if cw_qty and isinstance(cw_qty, list):
                                cw_qty = cw_qty.pop(0)
    ...
    def _create_extra_move(self):
        if (
            ...
            return super(StockMoveModel, self)._create_extra_move()
        else:
            extra_move = self
                ...
                if self.exists() and not self.picking_id:
                    for move_line in self.move_line_ids.filtered(lambda ml: ml.qty_done):
                        if float_compare(move_line.qty_done, extra_move_quantity, precision_rounding=rounding) <= 0 or \
                            ...
                            move_line.copy(
                                default={'move_id': extra_move.id, 'qty_done': extra_move_quantity,
    ...
    def _action_assign(self):
        if (
            not self.env.user.has_group('tis_catch_weight.group_catch_weight')
        ):
            return super(StockMoveModel, self)._action_assign()

        assigned_moves = self.env['stock.move']
        partially_available_moves = self.env['stock.move']
        for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
            missing_reserved_cw_quantity = move.product_cw_uom_qty - move.reserved_cw_availability
            missing_reserved_uom_quantity = move.product_uom_qty - move.reserved_availability
            missing_reserved_cw_quantity = move.product_cw_uom._compute_quantity(missing_reserved_cw_quantity,
            ...
            if move.location_id.should_bypass_reservation() \
                ...
                if not move.move_orig_ids:
                    ...
                    for k, g in groupby(sorted(move_lines_in, key=_keys_in_sorted), key=itemgetter(*keys_in_groupby)):
                        ...
                        qty_done = 0
                        for ml in g:
                            cw_qty_done += ml.product_uom_id._compute_quantity(ml.cw_qty_done, ml.product_id.uom_id)
                    ...
                    for (location_id, lot_id, package_id,
                        ...
                        available_quantity = self.env['stock.quant']._get_available_quantity(
                            move.product_id, location_id, lot_id=lot_id, package_id=package_id, owner_id=owner_id,
                            strict=True)
                        if float_is_zero(available_cw_quantity, precision_rounding=move.product_id.uom_id.rounding):
                            continue
                        ...
                        if float_is_zero(taken_quantity, precision_rounding=move.product_id.uom_id.rounding):
                            continue
                        if float_is_zero(cw_need - taken_quantity, precision_rounding=move.product_id.uom_id.rounding):
                            assigned_moves |= move
                            break                                                                             
        ...
        for reserved_quant, cw_quantity in quants:
            ...
            if not to_update:
                if self.product_id.tracking == 'serial' or self.product_id.tracking == 'lot':
                    if cw_params and self.id in cw_params.keys():
                        serial_dict = cw_params.get(self.id)
                        if not isinstance(serial_dict, list):
                            serial_dict = []
                        serial_dict.append(cw_quantity)
                        catch_weight.add_to_context(self, {self.id: serial_dict})
                    else:
                        serial_dict.append(cw_quantity)
                        catch_weight.add_to_context(self, {self.id: serial_dict})
                else:
