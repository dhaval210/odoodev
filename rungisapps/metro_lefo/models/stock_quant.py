...
class StockQuant(models.Model):
    ...
    def _get_available_quantity(self, product_id, location_id, lot_id=None, package_id=None, owner_id=None, strict=False, allow_negative=False):
        ...
        """
        self = self.sudo()
        quants = self.with_context(self.env.context)._gather(product_id, location_id, lot_id=lot_id, package_id=package_id, owner_id=owner_id, strict=strict)
    ...
    def _gather(self, product_id, location_id, lot_id=None, package_id=None, owner_id=None, strict=False):
        removal_strategy = self.with_context(
            self.env.context)._get_removal_strategy(product_id, location_id)
        ...
        query_str = 'SELECT "%s".id  FROM ' % self._table + from_clause\
                    + \
                    where_str + " ORDER BY "+ removal_strategy_order
    ...
    def _get_removal_strategy_order(self, removal_strategy):
        if removal_strategy == 'lefo':
            return 'use_date DESC, in_date, id'
    ...
    def _get_removal_strategy(self, product_id, location_id):
        ...
        if ctx.get('partner', False):
            part = int(ctx['partner'])
            partner_id = self.env['res.partner'].search([('id', '=', part)])
            if partner_id.removal_strategy_id:
                return partner_id.removal_strategy_id.method
